package com.geeklib.ether.ci.controller;
...
public class BuildController {
    ...
    @PostMapping("/s2i/{projectName}/{applicationName}")
    public ResponseEntity<String> createImage(@Valid @RequestBody BuildInfoS2i buildinfoS2i, @PathVariable String projectName, @PathVariable String applicationName) throws IOException{
        buildinfoS2i.setProjectName(projectName);
        buildinfoS2i.setName(applicationName);
    ...
    public ResponseEntity<BuildInfo> getBuildInfo(@PathVariable String projectName, @PathVariable String applicationName, @PathVariable long buildNumber) {
        BuildInfo buildInfo = buildService.getBuildInfo(projectName, applicationName, buildNumber);
    ...
    public ResponseEntity<List<BuildInfo>> listBuildInfo(@PathVariable String projectName, @PathVariable String applicationName){
        List<BuildInfo> buildInfos = buildService.listBuildInfos(projectName, applicationName);
        return ResponseEntity.ok(buildInfos);
    }
    ...
    @PostMapping("/b2i/{projectName}/{applicationName}")
    public ResponseEntity<Object> createImage(){

        return ResponseEntity.ok(null);
    }

    @GetMapping("/docker/healthz")
    public ResponseEntity<Info> healthz(){

        return ResponseEntity.ok(buildService.healthz());
    }

    @GetMapping("/{projectName}/{applicationName}/maxBuildNumber")
    public ResponseEntity<Long> getMaxBuildNumber(@PathVariable String projectName, @PathVariable String applicationName){
        return ResponseEntity.ok(buildService.getMaxBuildNumber(projectName, applicationName));
    }
